    ...
    _DeskTop = function (langlist) {
        ...
        */
        that.setCurrentOpenedWindowTitle = function (key, title) {
            var set_xdialogTitle_callback = $("body").find("iframe[src='" + key + "']").data("set_xdialogTitle_callback");
            if (typeof (set_xdialogTitle_callback) == "function") set_xdialogTitle_callback(title);
            //that.checkToSetOrEmptyDeskTitleBar();
        }
        /**
        * 页面请求窗口显示菜单
        ...
        */
        that.getXdialogParam = function (winid) {
            var xdialog = that.dialog.findXdialog(winid);
            if (xdialog) {
                return xdialog.getParam();
            }
        }
        /**
        * 设置对话框工具条
        * @method setXdialogToolBar
        * @param {string} winid 窗口ID
        * @param {json}   params 工具条描述对象
        */
        that.setXdialogToolBar = function (winid, params) {
            var xdialog = that.dialog.findXdialog(winid);
            if (xdialog && params) {
                return xdialog.setToolBar(params);
            }
        }
    };
    /*实例原型成员列表*/
