...
(function ($, XDialog) {
    ...
    */
    _DeskTop = function (langlist) {
        ...
        */
        ...
        /*openedEventHandler*/function (dialog, args) {//打开后调用，关于“窗口”菜单的配置
            ...
            if (!opened_win_li.is("li")) {
                ...
                opened_win_li.on("click", function () {dialog.open();});
            }
        },
                ...
                if (_iswin) {
                    ...
                    if (!args) {
                       ...
                       _hassearch = appbtn.data("win-hassearch"), _title = appbtn.data("win-title");
                        ...
                        args = { src: _src, dropEnable: true, mode: _winsize, hasmenu: _hasmenu, hassearch: _hassearch, title: _title };
                        ...
                        var handler = that.dialog.openWin(args);
                        appbtn.data(WINDIW_ARGS, handler.args)
                    }
                    else {
                        that.dialog.openWin(args)
                    }
        ...
        */
        that.setXdialogMenu = function (winid, menus) {
            var xdialog = that.dialog.findXdialog(winid);
            if (xdialog) {
                xdialog.args.hasmenu = true;
                xdialog.createMenu(menus, true);
            }
        }
        /**
        * 设置对话框参数
        ...
        */
        that.setXdialogParam = function (winid, params) {
            var xdialog = that.dialog.findXdialog(winid);
            if (xdialog && params) {
                xdialog.setXdialogParam(params)
            }
        }
        /**
        * 获取对话框参数
        ...
        * @param {string} winid 窗口ID
        */
        that.getXdialogParam = function (winid) {
            var xdialog = that.dialog.findXdialog(winid);
            if (xdialog) {
                return xdialog.getXdialogParam();
            }
        }
    };
    /*实例原型成员列表*/
        ...
        $("#desktop_bar a[data-iswin='True']").on("click", function () {
            ...
            if (src) {
                ...
                if (!args) {
                    ...
                    var handler = desk.dialog.openWin(args);
                    btn.data(WINDIW_ARGS, handler.args);
                }
                else {
                    desk.dialog.openWin(args)
                }
            }
            return false;
        })
