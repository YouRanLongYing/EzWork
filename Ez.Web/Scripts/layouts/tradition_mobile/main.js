...
(function ($, EzDialog) {
    var _DeskTop = function (langlist) {
        ...
        (function (desk) {
            ...
            desk.topBarBtns.click(function () {
                if (!desk.top_menu_btn_compress.is(":hidden")) {
                    desk.top_menu_btn_compress.trigger("click");
                }
                _click_event($(this));
            })
    ...
    _DeskTop.prototype = {
        ...
        initVar: function () {
            ...
            this.top_menu_btn_compress = $("#menu_btn_compress");
            this.iframe_top_nav = $("#iframe_top_nav");
            this.topBox = $("#iframe_header");
            this.body = $("#iframe_body");
            this.leftMenu = $("#iframe_body_left");
            this.rightBox = $("#iframe_body_right");
            ...
            this.system_btn_loginout = $("#system-btn-loginout");
        },
        ...
        setLayout: function () {
            ...
            this._slide_hidden_len = this.body.position().left;
            ...
            this.loginout_li = this.iframe_top_nav.find("ul").find("[append='loginout']").eq(0);

            if (this.top_menu_btn_compress.is(":hidden")) {
                this.iframe_top_nav.removeClass("iframe-header-menucompress am-collapsed am-collapse"); //.removeClass("am-collapsed").removeClass("am-collapse");
                if (this.loginout_li.length != 0) {
                    this.loginout_li.remove();
                    delete this.loginout_li;
                } 
                this.system_btn_loginout.fadeIn();

            }
            else {
                if (this.loginout_li.length==0) {
                    this.iframe_top_nav.find("ul").append("<li append=\"loginout\"><a href=\"/window/loginout\">退出</a><li>");
                }
                this.system_btn_loginout.fadeOut();
                this.iframe_top_nav.addClass("iframe-header-menucompress am-collapsed am-collapse");
            }
        },
        ...
        leftMenuSwitch: function () {
            ...
            if (this.menu_animate_play_over) {
                ...
                _l = that.body.position().left,
                _speed = "fast";
                that.menu_animate_play_over = false;
                if (_l == 0) {
                    var _newwidth = _rbw + _mlw;
                    that.body.stop().animate({ left: -_mlw, width: _newwidth }, _speed);
                    that.rightBox.stop().animate({ width: _rw + _mlw }, _speed, function () {
                        that.menu_animate_play_over = true;
                    });
                    that.switchbar.stop().animate({ "right": "-17px" }).removeClass("am-icon-toggle-left").addClass("am-icon-toggle-right");
                }
                else {
                    that.body.stop().animate({ left: 0, width: _rbw }, _speed);
                    that.rightBox.stop().animate({ width: _rw - _mlw }, _speed, function () {
                        that.menu_animate_play_over = true;
                    });
                    if (that.leftMenu.position().left > 0)
                        that.leftMenu.stop().animate({ left: 0 }, _speed)
                    that.switchbar.stop().animate({ "right": "1px" }).removeClass("am-icon-toggle-right").addClass("am-icon-toggle-left");
                }
            }
        },
        /**
        ...
        */
        setEzDialogMenu: function (winid, _menus, from) {//有对应页面调用
            var that = this;
            var htmlpool = [];
        ...
        */
        setEzDialogParam: function (id, params) {
        ...
        */
        getEzDialogParam: function (winid) {
